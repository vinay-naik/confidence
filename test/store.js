...
const tree = {
    ...
    key7: {
        $filter: "env",
        production: [{ animal: "chicken" }, { animal: "dog" }],
        staging: [{ animal: "cow" }],
        $base: [{ animal: "cat" }],
    },
    key8: {
...
describe("get()", () => {
    ...
    get("/key1", "abc");
    get("/key2", 2, null, [
        { filter: "env", valueId: "$default" },
        { filter: "platform", valueId: "$default" },
    ]);
    get("/key2", 1, { platform: "ios" }, [
        { filter: "env", valueId: "$default" },
        { filter: "platform", valueId: "ios" },
    ]);
    ...
    get("/key5", {});
    get("/key6", { animal: "chicken", color: "orange" }, {}, [{ filter: { $env: "NODE_ENV" }, valueId: "production" }], {
        NODE_ENV: "production",
    });
...
describe("validate()", () => {
    ...
    validate("out of order range array elements", {
        key: {
            $filter: "a",
            $range: [
                { limit: 11, value: 2 },
